...
class GameSaveMgr(object):
    def __init__(
        self, program_name, p7zip_path, save_folder, backup_folder, rungame_url
    ):
    ...
    def save_game(self, filename: str = "", overwrite: bool = True):
        ...
        if filename == "":
            ...
            self.backup_file = os.path.join(
                self.backup_folder, filename + "_" + timestamp
            )
    ...
    def backup_list(self):
        ...
        for file in file_list:
            backup_list.append(
                [file, os.path.getctime(self.backup_folder + "\\" + file)]
            )
...
game = GameSaveMgr(p_name, p7z_path, sv_folder, bak_folder, rg_url)
